...
class Calculadora(tk.Tk):
    def __init__(self):
        ...
        # caja de texto
        self.entrada = None
        # string var lo utilizamos para actualizar el input
    ...
    def _creacion_componentes(self):
        # creamos un frame
        entrada_frame = tk.Frame(self, width=430, height=50, bg='grey')
        entrada_frame.pack(side=tk.TOP)
        # caja de texto
        entrada = tk.Entry(entrada_frame, font=('arial', 18, 'bold'),
                           textvariable=self.entrada_texto, width=32, justify=tk.RIGHT)
        entrada.grid(row=0, column=0, ipadx=1, ipady=10)

        # creamos segundo frame para la parte inferior
        botones_frame = tk.Frame(self, width=430, height=450, bg='grey')
        botones_frame.pack()

        # primer renglon
        boton_limpiar = tk.Button(botones_frame, text='C', width=37, height=3,
                                  bd=0, bg='#eee', cursor='hand2', command=self. _evento_limpiar)
        boton_limpiar.grid(row=0, column=0, columnspan=3, ipadx=1, pady=1)
        # boton de dividir
        boton_dividir = tk.Button(botones_frame, text='/', width=10, height=3, bd=0,
                                  bg='#eee', cursor='hand2', command=lambda: self._evento_click('/'))
        ...
        # quinto renglon
        boton_cero = tk.Button(botones_frame, text='0', width=24, height=3, bd=0, bg='#fff',
                               cursor='hand2', command=lambda: self._evento_click(0))
        boton_cero.grid(row=4, column=0, columnspan=2, padx=1, pady=1)
        boton_punto = tk.Button(botones_frame, text='.', width=10, height=3, bd=0,
                                bg='#eee', cursor='hand2', command=lambda: self._evento_click('.'))
        boton_punto.grid(row=4, column=2, padx=1, pady=1)
        boton_igual = tk.Button(botones_frame, text='=', width=10, height=3, bd=0,
                                bg='#eee', cursor='hand2', command=self._evento_evaluar)
    ...
    def _evento_click(self, elemento):
        # concatenamos el nuvo elento a la expresion ya existente
